...
public class GlobalProjectorManager : MonoBehaviour {
	...
	public static bool GlobalFlipX {
		set {
			bool changed = (_Instance._GlobalFlipX != value) ? true : false;
			_Instance._GlobalFlipX = value;

			if (changed) {
				_Instance.ForceShadowDummyRebuild();
			}

		}
		
		get {
			return _Instance._GlobalFlipX;
		}
	}
	
	bool _GlobalFlipX = false;

	public static bool GlobalFlipY {
		set {
			bool changed = (_Instance._GlobalFlipY != value) ? true : false;
			_Instance._GlobalFlipY = value;
		...
		get {
			return _Instance._GlobalFlipY;
		}
	}
	
	bool _GlobalFlipY = false;
	...
	public static GlobalProjectorManager NoNullCheckGet()
	{
		return _Instance;
	}
	...
	void ForceShadowDummyRebuild() {
		ForceShadowDummyRebuild(_ShadowProjectors);
		ForceShadowDummyRebuild(_LightProjectors);
	}

	void ForceShadowDummyRebuild(List<ShadowProjector> projectors) {
		foreach (ShadowProjector shadowProjector in projectors) {
			shadowProjector.OnUVRectChanged();
		}
	}
	...
	void Update() {
		...
		for (int i = 0; i < _ShadowReceivers.Count; i++) {
			...
			if (receiver.IsTerrain() && !receiver.IsStandardTerrain()) {
	...
	void PreventDynamicBatchUnity5() { 
		#if UNITY_5_0 || UNITY_5_1 || UNITY_5_2 || UNITY_5_3 || UNITY_5_4 
		_BatchBreak++;
		_BatchBreak = Mathf.Clamp(_BatchBreak, 0.0f, float.MaxValue);
		
		_MBP.SetFloat("_Unity5BugFix", _BatchBreak);
		#endif
	}
